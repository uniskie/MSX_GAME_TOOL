...
- `HYDELIDE3.BAS`   ...本ツール
- `HYDLIDE3.BIN`    ...マシン語ファイル `0xA000`にロードされる  
- `HYD3-EXP.BIN`    ...マシン語ファイル `0xFA75`にロードされる  
...
- BASICに戻った後 **[C] CONTINUE** で中断した状態から再開できる  
- BASICに戻って **[M]** を押して強制セーブした場合は、  
  **[G] PLAY GAME** で開始して、セーブデータをロード開始する事  
- **[W]** で中断状態をファイルへ保存  
...
- MSX2版は共通パレットのみ復元  
  → ゲーム中のMulti-Windowメニュー([RETURN]またはトリガ2で開くもの)を閉じる時に
  パレットが復元される。  

- ゲーム中以外の保存が不安定。  
  タイトルデモ/エンディングへの復帰は画面がおかしくなり、暴走の可能性あり。  
...
ゲーム中のRAMのワークエリアを書き換えるものなので、一度ゲームを実行してからRAMの状態を維持してBASICに戻る必要があります。

- ※ 重いのでターボR推奨

### 手順

1. （あれば）PAC/FMPACを挿し、**ハイドライド3のカートリッジは挿さず**にMSX本体の電源投入
2. `RUN"HYDLIDE3.BAS"` でハイドライド3起動ツールを実行。
3. ツールが起動したらカートリッジを後挿しする。  
4. **[L] LOAD CHARACTERS** または **[G] PLAY GAME**  
   ゲーム開始後、**[STOP]**→**[CTRL+@]**  
   (ゲームを中断してツールに復帰)  
5. **[E] CHARA DATA EDIT** でキャラクターエディタ開始。
6. **[CTRL+STOP]** でキャラクターエディタ終了。
7. `RUN"HYDLIDE3.BAS"` でハイドライド3起動ツールに戻る。
8. **[G] PLAY GAME** または、  
   **[S] SAVE CHARACTERS** → **[L] LOAD CHARACTERS**  
   で、ゲーム開始。  

   - ※ **CONTINUE**の場合、編集したセーブデータは破棄されます。

   - ※ 編集したらセーブ推奨。  
     `"HYDLIDE3.BAS"`ツールを起動したときに、  
     **[G] CONTINUE GAME** になっている場合は、  
     **[S] CHARACTER SAVE** で、データセーブをし、  
     **[L] CHARACTER LOAD** で、データロード開始。  

> フリーエリアが足りなくて読み込めない場合は  
...
**[S] SAVE CHARACTERS** して **[L] LOAD CHARACTERS** することで、PACのSRAMにも保存されます。

> **【警告】**  
>
> **[C] CONTINUE GAME**や **[R] STATE LOAD**では、
> 編集したキャラクターデータはゲームに反映されず、**編集した内容が失われます。**
> 
### キャラクターデータのアドレス
...
address:0xC000
size:0x400
保存キャラクターデータの受け渡しバッファ（4キャラ分）  
```

ここを読み書きしてツール(`HYDLIDE3.BAS`)に渡すと
キャラクターデータが書き換わります。  
(チェックサムを調べて合っていれば取り込み)

### チェックサム計算

1キャラ256バイトで、最後の1バイトはチェックサム。

チェックサムの計算式は(+0x0000)～(+0x00FE)の合計+0x4A  

``` basic
CLEAR 200,&HFB00
A$="FE02C02AF8F706FFAF862310FCC64A6F260022F8F7C9"
FOR I=1TO LEN(A$)-1 STEP 2:POKE &HBF00+I\2,VAL("&H"+MID$(A$,I,2))
PRINT".";:NEXT:DEFUSR5=&HBF00
A=USR5(データアドレス)  
```  
...
## MSX2版：「中断・再開」機能について補足

VDPアクセスにBIOSを使用していないプログラムの場合は、パレットの値を後から取得する方法がありません。
そのため、再開時はメニュー画面用パレットで再開しています。

よってMSX2版では、以下のような注意点や制限があります。

- **注意点**
  - なるべくメニュー画面で中断する。（パレットが固定）
  - フェードアウト中に中断・再開をしない。（パレットが不定なので再現困難）
  - ED/OPで中断・再開をしない。（非対応）
...
- **ディスクの読み書きが遅い。**  
  ...
  - バッファが0x2000しかないのでさらに遅い。  
  - 裏RAM0x6000-0x7FFFが空いているがディスクアクセス中は使いづらい。

### MSX2版ステートロード時のカラーパレット

ハイドライド3はVDPを直接アクセスしているため、パレットは読みだせませんので、
...
> ※ $0000はアセンブラでの16進数表記の一つ。C言語等の0x0000に相当する

→ 結果：バンク番号だけでは識別できない様子

現在の表示モード（OP/ED/MAP/SHOP/MENU）とマップ番号のワークエリアが分かれば判定は可能かもしれませんが、OP/EDは呼び出し先だけ異なる可能性が高いので少し面倒くさそうですね。

----

# おまけ：キャラクターデータエディタ 詳細解説
...
HYDLIDE3.BAS実行後だとフリーエリアが足りなくて読み込めない場合があるので、その場合は
...
現在のバンク選択状態を知る方法は「現在のメモリの状態を調べる」事になる。
...
## HYDLIDE3 と HYD3-EXPのやりとり

### ゲーム中の`$C000`-`$FFFB`領域

BASIC時は裏RAM `$0000`-`$3FFB`に置かれる。  
（ゲーム実行時に入れ替わる）  
（裏RAM入れ替え時、16BIT値のチェックサムを計算し`$3FFC`に格納）

ゲーム時の`$C000`-`$FFFB`の内容（BASIC時の裏RAM `$0000`-`$3FFB`）は  
状態セーブロード時に読み書きされる。  
（ほかにはVRAMデータも読み書きされる）

### ゲーム拡張割り込み処理 スタックエリア

BASIC PLAY文用のキューバッファを利用する
...
``` text
 VDP REGISTER   0       4       8       12  xxxx16xxxxxx20xxxx23
...
バンク切り替えレジスタとページの対応:
$6000: page0 $4000-$5FFF を切り替え  
$6800: page1 $6000-$7FFF を切り替え  
$7000: page2 $8000-$9FFF を切り替え  
$7800: page3 $A000-$BFFF を切り替え  

バンク切替ワークエリア:
$DEBB: page0 (reg.$6000 adr.$4000-$5FFF)
$DEBC: page1 (reg.$6800 adr.$6000-$7FFF)
$DEBD: page2 (reg.$7000 adr.$8000-$9FFF)
...
※ 割り込み中などはこのルーチンを使わずに直接バンク切り替えなので要注意  
...
``` text
bank $01/$1400-$1aff = $3400-$3aff: 半角英数・全角カナフォント  
bank $01/$1b00-$1fff = $3b00-$3fff: 漢字、T&Eロゴ、全角英数
```

メインフォント

``` text
bank $3f/+$0000-$0fff = $7e000-$7efff: 漢字フォント
bank $3f/+$1000-$16ff = $7f000-$7f6ff: 半角英数・全角カナフォント  
```

### MSX2 ver

MSX2版ではOP/ED用フォントは無い。（メインの漢字フォントを流用）  
ANKフォントと漢字フォントがそれぞれ1つずつ離れたバンクに存在する。

``` text
bank $15/+$1400-$1fff = $2b400-$2bfff: 半角英数・全角カナフォント  
bank $3f/+$0000-$1fff = $7e000-$7ffff: 漢字フォント
```
...
``` text
20H～FDHが半角文字
メイン漢字：FEH、FFHは漢字の接頭辞
OP/ED漢字：07Hの次のバイト-20H
...
- メッセージデータの形式はMSX1/2共通
- MSXのみ専用フォントあり。
- MSX2版はメインメッセージのフォントを代用  
  (漢字のみ・T&ESOFTロゴなし)
