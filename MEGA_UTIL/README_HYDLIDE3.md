...
1. `RUN"HYDLIDE3.BAS"` で起動ツールを実行
2. ツールが起動したらカートリッジを後挿しする  
3. **[L] LOAD CHARACTERS** または **[G] PLAY GAME**  
   ゲーム開始後、**[STOP]**→**[CTRL+@]**  
   (ゲームを中断してツールに復帰)  
4. **[E] CHARA DATA EDIT** でキャラクターエディタ開始
5. **[CTRL+STOP]** でキャラクターエディタ終了
6. `RUN"HYDLIDE3.BAS"` でハイドライド3起動ツールに戻る
7. **[G] PLAY GAME** または、  
...
- パレットの保存方法がないのでメニュー画面用パレットで再開している。
- 以下の注意点がある。
  - フェードアウト中に中断～再開をしないこと。
  - なるべくメニュー画面で中断する。
...
ハイドライド3はVDPを直接アクセスしているため、パレットは読みだせません。  
（状態保存できません）
