...
■ （検討事項）コナミ系を汎用化したい
  ...
  案）
    ...
    2) ゲーム実行時は、
       DI
       IM 1
       ぺージ1(4000H)とページ2(8000H)のスロットをゲームのスロットに切り替え
       スタックポインタをゲームのスタックポインタアドレスに設定
       HLに自分の割り込み処理アドレスを入れてLD  (H_KEYI+1),HLへジャンプ
       もしくは、自分でH_KEYIを設定して、LD  (H_KEYI+1),HLの次のアドレスへジャンプ。
    ...
    希少例：
    LD (4000H),A  ;32 00 40 ;BANK0 切替
    LD (6000H),A  ;32 00 60 ;BANK1 切替
    LD (8000H),A  ;32 00 80 ;BANK2 切替
    LD (B000H),A  ;32 00 B0 ;BANK3 切替

   （※ 実際にBANK0を切り替えているケースは見たことが無いので
        BANK0切り替えをしているソフトは存在しないかもしれない。）

  コナミ8kバンク/SCCメガロムの場合
	（牌の魔術師を除く）
    $4000 (2): 'AB'
    40002 (2): 起動時のジャンプアドレス
    $4010 (2): 文字列'CD'か'EF'(SHALOM)
