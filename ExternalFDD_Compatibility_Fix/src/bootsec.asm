...
;   きゃんきゃんバニースペリオールでは、
;   ページ1をRAMに切り替えるときに、
;   「インタースロットコールでENASLTを呼び出す」
;   という愉快なバグを入れ込んでいるためです。
;
;   （ディスクブート時は、RAMの0144HにもENASLTがあるので
;     インタースロットコールで呼び出す必要がありません。）
;
;   インタースロットコールの戻りで
;   ページ1の基本スロット番号が呼び出し前のスロットに
;   戻されてしまうのです。
;   （内蔵FDDとRAMは基本スロットが同じなので問題が無い）
;
; 対処:
...
Start_C:
    ...
    ; が入っている。
    ; スロットはページ0から順に
    ;   RAM、DISKROM、RAM、RAM となっている。

    ; テクニカルガイドブック第4版やデータパックでは
    ; DEに入っている値が「ページ1をRAMに変えるフック」(F36BH)
    ; と、書いてあるが、間違い。
    ; 正しくは 「ページ1をDISKマスタースロットに切換えるフック」(F368H)

    ; テクニカルガイドブック第4版では
    ; Aレジスタに起動時のドライブ番号とあるが、
